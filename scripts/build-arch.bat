...
%WINDEPLOYQT_CMD% --dir %DEPLOY_FOLDER% --%BUILD_CONFIG% --qmldir %SOURCE_ROOT%\app\gui --no-opengl-sw --no-compiler-runtime --no-sql %WINDEPLOYQT_ARGS% %BUILD_FOLDER%\app\%BUILD_CONFIG%\OmnuvClient.exe
if !ERRORLEVEL! NEQ 0 goto Error
...
if "%SIGN%"=="1" (
    ...
    set FILES_TO_SIGN=%BUILD_FOLDER%\app\%BUILD_CONFIG%\OmnuvClient.exe
...
if "%ML_SYMBOL_STORE%" NEQ "" (
    ...
    if !ERRORLEVEL! NEQ 0 goto Error
    symstore add /r /f %BUILD_FOLDER%\app\%BUILD_CONFIG%\OmnuvClient.exe /s %ML_SYMBOL_STORE% /t Moonlight
    if !ERRORLEVEL! NEQ 0 goto Error
)
