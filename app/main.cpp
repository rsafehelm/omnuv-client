...
LONG WINAPI UnhandledExceptionHandler(struct _EXCEPTION_POINTERS *ExceptionInfo)
{
    ...
    swprintf_s(dmpFileName, L"%ls\\OmnuvClient-%I64u.dmp",
...
int main(int argc, char *argv[])
{
    ...
    // Omnuv: the product's own identity, matching the binary (OmnuvClient) and
    // the icon. This decides the window title, the QSettings location and
    // QStandardPaths, so leaving it would give an OmnuvClient.exe whose window
    // said "Moonlight" and whose settings lived under upstream's key. A fork
...
#endif
    {
        s_LoggerFile = new QFile(tempDir.filePath(QString("OmnuvClient-%1.log").arg(QDateTime::currentSecsSinceEpoch())));
    ...
    QStringList existingLogNames = tempDir.entryList(QStringList("OmnuvClient-*.log"), QDir::NoFilter, QDir::SortFlag::Time);
